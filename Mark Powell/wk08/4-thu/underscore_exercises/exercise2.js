...
var lowerCaseWord = _.map(words,function(word){
  return word.toLowerCase();
});
var sorted_again = _.groupBy(lowerCaseWord, function(word){
  return word[0];
});

console.log(sorted_again);
// Check to see if all the words have more than 3 characters
var longWords = _.every(words, function(word){
...
var uid = _.map(people, function(person){
  return person.uid;
});
console.log(uid);
...
var split = _.countBy(numbers, function(number){
