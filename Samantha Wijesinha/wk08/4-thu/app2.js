...
var uid = _.sortBy(people, 'uid');
console.log(uid)
// Group the random words by the lower case version of their first letter
var splitArray = _.map(words, function(word){return word.toLowerCase() });
var sorted = _.groupBy(splitArray, function(word){ return word[0]; });

console.log(sorted)
// Check to see if all the words have more than 3 characters
var wordCheck = _.every(words, function(word) { return word.length > 3  });

// Check if some words are over twenty characters long
var wordCheck20 = _.every(words, function(word) { return word.length > 20  });
console.log(wordCheck20)
// Get an array of all of the uids in people
var uids = _.map(people, function(person){return person.uid });
console.log(uids)
// Find the person with the highest uid
var highUid = _.max(people, function(person){ return person.uid; });
console.log(highUid)
// Return an object that says how many even numbers and how many odd numbers there are in numbers
var oddEven = _.countBy(numbers, function(num) {
